...
`reference/desktop/` preserves the supplied images, composition screenshot, original paths and hashes. `prepare_desktop.py` copies the three unedited panel images byte-for-byte and removes the notification’s magenta backing with an alpha matte and spill removal. Its white speech bubble, tail, lettering and icon remain. `godot/assets/desktop/` holds these classified prototype outputs. No additional generation or spend. The approved Muse map title/frame and all map artwork/data are unchanged.
...
## Moving the windows and pending Muse copy edits
...
Muse edits are **pending** for itinerary `Collection Hightlights` (owner spelling), both `Collection number` labels with the existing zeros preserved, minimap `MiniMap`, and notification `Recently Added!`. All three initial OpenRouter requests returned explicit HTTP 402 insufficient-credit rejections, with no outputs. No request was retried, and no text asset was replaced. Versioned reference crops, prompts and rejection records are in `generation/*-text-*`; the ledger distinguishes these rejections from completed generations. Restore OpenRouter credits before submitting new versioned passes.
