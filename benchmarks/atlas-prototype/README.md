...
The atlas now runs in its own Godot viewport inside the pale cyan reference frame. Drag the title bar to move it; drag any edge or corner to resize it. The left title icon collapses/restores the map, and the right icon locks/unlocks moving and resizing. Browser resizing fits the window to the available screen. Map zoom and drag stay inside the content area, and the phone toolbar hides Full sheet when space is limited. The complete frame is extracted from the supplied screenshot, including the header gradient, title/icons, rounded corners, border shading and padding. Its embedded display profile is converted to sRGB. Corners and lettering retain their source dimensions when the window resizes; only straight runs and the empty header band stretch. On narrow phone screens the complete frame uses 50% scale, with title text clipped rather than squeezed.
...
## Supplied desktop panels — September 8

The desktop now follows the supplied 9:31 AM composition: the minimap/calendar sits above the vertical itinerary on the left, chat sits below them, the interactive map occupies the center, and the notification sits at bottom right. All four supplied assets are included; the separate “2ND ALBUM” placeholder is omitted. Each panel keeps its original orientation and aspect ratio. The desktop composition scales to fit the browser; phones keep the map usable at the top and place the panels below it. Map dragging, resizing, collapse and zoom remain live. The added panels are supplied raster artwork, with no chat/calendar/trade backend implied.

`reference/desktop/` preserves the supplied images, composition screenshot, original paths and hashes. `prepare_desktop.py` preserves the chat image byte-for-byte, applies the tight Muse lettering edits described below, and removes the notification’s magenta backing with an alpha matte and spill removal. Its white speech bubble, tail, lettering and icon remain. `godot/assets/desktop/` holds these classified prototype outputs. The approved Muse map title/frame and all map artwork/data are unchanged; the panel lettering edits and their spend are recorded below.

Current composition verification: `node benchmarks/atlas-prototype/playtest-layout.mjs`, `python3 benchmarks/atlas-prototype/prepare_desktop.py`, and `python3 benchmarks/atlas-prototype/verify_assets.py`. The layout playtest covers exact reference desktop placement, all four assets, no initial overlaps at desktop/laptop/phone sizes, real region selection, map zoom/drag, window resize/drag/collapse and phone pinch. Screenshots and runtime results are in `evidence/layout/`. Earlier standalone-window playtests and frame comparisons remain as historical regression evidence; their fixed viewport geometry predates this composition.


## Moving the windows and Muse copy edits

Drag any of the four supplied raster panels to move it; drag the map title bar to move the map. Clicking a window brings it forward. Overlapping input follows the top window, and scrolling over a raster panel does not zoom the map beneath it. Mouse and touch movement clamp windows to the screen and clear on release. The existing map resize/collapse/lock controls remain; locking the map does not lock the other panels. Browser resize restores the responsive reference composition.

Muse now supplies the itinerary title `Collection Hightlights` (owner spelling), both `Collection number` labels with their existing zeros preserved, minimap `MiniMap`, and notification `Recently Added!`. Three reference edits completed through OpenRouter for $0.03 after the earlier credit rejections were resolved. Total prototype generation spend is $0.05 for five outputs; the three earlier HTTP 402 rejections produced no images. Versioned references, prompts, outputs and spend records remain in `generation/*-text-*`.

Assembly uses the generated glyphs in explicit text rectangles, restores the source background and original ink color, and returns the two upright edit references to their original vertical orientation. No pixels outside those rectangles change. Both original zero numerals and every alpha value remain exact. `prepare_desktop.py` reproduces the assets and writes the preservation assertions to `evidence/text-fidelity.json`; `evidence/text/` contains upright comparison views. Chat remains byte-identical. The current composition/movement playtest is rerun against the updated Web export.
