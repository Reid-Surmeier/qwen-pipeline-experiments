...
const cdp=await context.newCDPSession(rawPage);const origin=(await state()).window.map_rect;const points=(a,b)=>[{x:a+origin[0],y:440+origin[1],id:0},{x:b+origin[0],y:440+origin[1],id:1}];const start=await state();await cdp.send('Input.dispatchTouchEvent',{type:'touchStart',touchPoints:points(140,240)});
...
const pan=await state();await cdp.send('Input.dispatchTouchEvent',{type:'touchStart',touchPoints:[{x:200+origin[0],y:500+origin[1],id:0}]});await cdp.send('Input.dispatchTouchEvent',{type:'touchMove',touchPoints:[{x:280+origin[0],y:560+origin[1],id:0}]});await cdp.send('Input.dispatchTouchEvent',{type:'touchEnd',touchPoints:[]});await page.waitForTimeout(500);assert.notDeepEqual((await state()).position,pan.position);assert.equal((await state()).touches,0);assert.equal((await state()).orphan_dots,0);assert((await state()).vertical_pan_locked || (await state()).south_edge<=2700.01);
...
assert((await state()).viewport[0]<322);assert.equal((await state()).orphan_dots,0);await shot('mobile-window-resized');
