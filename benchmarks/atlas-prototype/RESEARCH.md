...
## Hawaiian islands and map window

Diagnosis: the geographic cleanup removed land polygons below 60 native square pixels unless near an original city; none of the original 396 city annotations was in Hawaii. Most Hawaiian islands therefore disappeared during the detail transition, and the later catalog rejected their city coordinates for lacking retained land. Preserve the eight main island polygons within the sourced Hawaiian extent while keeping the existing cleanup elsewhere. Rebuild their detail tile and re-evaluate GeoNames anchors; give Honolulu and the three main island towns early thresholds.

The original `trade-minimized.png` title plate in the existing Godot replica matches the supplied window reference. Reuse its title text and icons through AtlasTexture regions. Native Controls draw the rounded cyan frame; a [SubViewportContainer](https://docs.godotengine.org/en/stable/classes/class_subviewportcontainer.html) with stretch enabled resizes its SubViewport and routes local map input. Embedded subwindows keep the region picker inside the map. This avoids resizing a rendered bitmap. Edge/title interactions belong to the frame, while the existing atlas retains its camera and clipping calculations against its inner viewport.

## Exact frame fidelity correction

The owner rejected the approximate three-stop gradient, radius 20, thin border padding, and separate older title plate. The supplied screenshot is the authority. It carries an embedded display profile; raw RGB samples differ from its displayed sRGB colors. A deterministic ImageCms conversion preserves that appearance, then the reference frame is extracted with the live map interior made transparent. No pixels are synthesized for the frame. Source SHA/profile hash and crop geometry are recorded in `reference/window-source.json`.

Godot draws fixed source corners and title/icon regions, stretching only straight borders and the empty part of the header. The original bottom inner-corner border overlays the map, retaining its curvature. The native content inset is 36×94 pixels with a 42-pixel bottom rail, matching the source crop. Chrome scales uniformly to 50% below 750 screen pixels; lettering clips when necessary. Native-size and resized rendered headers/corners are checked directly against the sRGB-normalized source, in addition to desktop and phone input tests.
