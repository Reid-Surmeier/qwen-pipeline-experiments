...
func _ready() -> void:
	...
	for copy in [-1, 0, 1]:
		...
		_sprite(world_root, "world", shift, Vector2.ONE)
		_sprite(detail_root, "terrain", shift, Vector2.ONE)
	for region in atlas.regions:
		...
		for group in region.annotations:
			...
			annotations.append({"sprite": sprite, "at": Vector2(group.at[0], group.at[1]), "kind": group.kind, "region": region.id, "size": Vector2(group.rect[2], group.rect[3]), "scale": 10.0 / group.rect[3] if group.kind == "city" else 1.0})
	add_child(camera)
...
func _reset() -> void:
	...
	_resize()
	camera.position = Vector2(WIDTH / 2, HEIGHT / 2 - 45 / _fit_zoom())
...
func _zoom_at(factor: float, anchor: Vector2) -> void:
	...
	var next := clampf(camera.zoom.x * factor, _fit_zoom(), 4.0)
...
func _layout_annotations() -> void:
	...
	order.sort_custom(func(a: Dictionary, b: Dictionary) -> bool:
		var pa := (0 if a.kind == "badge" else (1 if a.kind == "city" else 2)) + (0 if a.region == selected else 3)
		var pb := (0 if b.kind == "badge" else (1 if b.kind == "city" else 2)) + (0 if b.region == selected else 3)
		return pa < pb)
	...
	for item in order:
		...
		sprite.visible = rect.intersects(Rect2(Vector2.ZERO, _viewport_size()))
		...
		if collides and item.kind != "city":
...
func _process(delta: float) -> void:
	var target := smoothstep(0.4, 0.85, camera.zoom.x)
	detail_alpha = move_toward(detail_alpha, target, delta * 4)
	detail_root.modulate.a = detail_alpha
	if mode == "atlas":
		var distance := INF
		for region in atlas.regions:
	...
	if camera.position != layout_position or camera.zoom.x != layout_zoom:
		_layout_annotations()
	status.text = "%s · %s · %.1f×" % [_region(selected).name if camera.zoom.x > 0.4 else "World", "Full sheet" if mode == "sheet" else ("Regional detail" if detail_alpha > 0.8 else "Overview"), camera.zoom.x / _fit_zoom()]
	state_timer += delta
	if state_timer > 0.2:
		state_timer = 0
		_publish_state()
